...
```
I chose the vancouver_trees dataset. I first chose the variables related to my research question which are genus_name and diameter_cm and then choose the rest randomly. The dataset is tidy since each row is an observation, each column is a variable and each cell is a value.
<!----------------------------------------------------------------------------->
...
```
I first untidy the data set by using pivot_wider(), it creates many columns about the type of tree, also it makes many of cells are NA. Then I use pivot_longer() to tidy the dataset to original one.
<!----------------------------------------------------------------------------->
...
1.  Previous: Does the genus of a tree affect its diameter?
    Updated: Does the species of a tree affect its diameter?
2.  Previous: What is the most planted tree (genus) in Vancouver?
...
<!--------------------------- Start your work below --------------------------->
In the previous milestone, I realized that there exits huge difference between different genus, now I am going to focus on different species in the same genus to see if there is difference among species.
<!----------------------------------------------------------------------------->
...
<!--------------------------- Start your work below --------------------------->
```{r}
# For question 1, I will choose the acer genus since acer is the most planted tree in vancouver.
...
acer_count
# Since there are more than 30 species, I will only select the planted_level with high
...
```
By looking at the boxplot above, the diameters are varies among species in acer genus.
...
```
In the original plot, the levels of column diameters_cm is randomly ordered. I use fct_order() created a data frame with increasing order of average diameters_cm.
...
vancouver_trees_subset
```
I use yearquarter() function to create a new column that store the quarter of a tree planted. It is useful since I can investigate does the quarter matters when we plant a tree. For this kind of question, I don't think a specific day is a good idea.
<!----------------------------------------------------------------------------->
...
**Research Question**: If there is statistically association between the species of tree and its diameter?
Null hypothesis: The diameters of different species in same genus are independent.
Alternative: The diameters of different species in same genus are dependent.
...
slr_results
```
I use tidy() to obtain the estimated coefficients, associated standard errors, t-statistic and p value. Based on the simple linear regression result above, we reject the null hypothesis. Thus, the diameters of a tree is statistically associated with the type of species in same genus.
<!----------------------------------------------------------------------------->
